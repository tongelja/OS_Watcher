...
sub parse_iostat {
    ...
    while (my $line = <$INPUT_FILE>) {
        ...
        if ($line =~ m{^\d+/\d+/\d+}mx) {                              ## zzz signifies the start of a new transaction in the log file
            $trans_number += 1;
            ($time) =  $line =~ m{(\d{2}:\d{2}:\d{2})}xms;
            $counter = 0;
            next;
        ...
        if ( $is_asm_valid && ! defined $disk{$device} ) {
            $cmd="$oracleasm_cmd querydisk /dev/$device" . "1 2>\&1" ;
            $std_out = `$cmd `;
            #printf "%s\n", "$cmd :  $std_out";
            ...
            if ( my ( $disk_name ) = $std_out =~ m/is\smarked\san\sASM\sdisk\swith\sthe\slabel.*"([\S]+)"$/xms ) {
                #printf "%s\n", "$device : $disk_name";
                $disk{$device}  = $disk_name ;
            } elsif (  length $std_out_02 > 0 ) {
                chomp $std_out_02 ;
                $disk{$device}  = $std_out_02 ;
            } else {
                $disk{$device}  = ' ' ;
            }
        }
        elsif ( $is_exa_valid && ! defined $disk{$device} ) {
            #$cmd="$cellcli_cmd -e 'list lun detail' | grep -B 1 '$device\$'  | grep -v '$device' | awk '{print \$2}'" . " 2>\&1" ;
            $cmd="$cellcli_cmd -e \"list celldisk where deviceName='/dev/$device' detail\" | grep ' name: '  | awk '{print \$2}'" . " 2>\&1" ;
            $std_out = `$cmd `;
            #printf "%s\n", "$cmd :  $std_out";
        ...
        elsif ( ! defined $disk{$device} ) {
            $disk{$device}  = ' ' ;
        }

        #printf "%s\n", "$disk{$device}, $device  :  $input_search";
        next if  ( $disk{$device} !~ /$input_search/  && $device !~ /$input_search/ );
 
        $record{$time}{"$device.$counter"}{disk} = $disk{$device};
...
sub parse_netstat {
    ...
    while (my $line = <$INPUT_FILE>) {
        ...
        if ($line =~ m{^\s+zzz.*$}mx) {                              ## zzz signifies the start of a new transaction in the log file
            $trans_number++;
            ($time) =  $line =~ m{(\d{2}:\d{2}:\d{2})}xms;
...
sub parse_top {
    ...
    while (my $line = <$INPUT_FILE>) {
        ...
        if ($line =~ m{^top.*$}mx) {                              ## zzz signifies the start of a new transaction in the log file
...
sub parse_ps {
    ...
    while (my $line = <$INPUT_FILE>) {
        ...
        if ( my ($flag,  $stat,      $uid,       $pid, 
                 $ppid,  $psr,       $cpu_util,  $priority,  
                 $nice,  $addr,      $sz,        $wchan,     
                 $stime, $tty,       $ps_time,   $cmd      ) = $line =~ m/^(\d+)\s+
                                                                        (\w+)\s+
                                                                        (\w+)\s+
                                                                        (\d+)\s+
                                                                        (\d+)\s+
...
sub print_headers {
    ...
    if ( $count%$page_size == 0 ) {
        if ( $data_type =~ m/VMSTAT_CPU/i ) {
            ...
            printf "\n$format",   'Time',           'Device',        'Disk',      'Rd Rq/s',       'Wt Rq/s',
